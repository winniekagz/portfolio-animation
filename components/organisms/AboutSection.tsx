...
import { Globe, Mail, Play, Users, Code2 } from "lucide-react";
...
const SOCIAL = [
  { Icon: Code2, label: "GitHub", href: "https://github.com/winniekagz" },
...
export function AboutSection() {
    ...
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: section,
          start: "top top",
          // Extra scroll budget so the pin holds until ALL right-side text
          // (including bio word stagger) is fully visible before releasing.
          end: "+=180%",
          pin: true,
          pinType: "transform",
          scrub: 1.8,
      ...
      // ── 6. Right panel rises up from below its mid-page anchor ──────────
      tl.fromTo(
        rightRef.current,
        { y: 40, opacity: 0 },
  ...
  return (
    ...
    >
      ...
      {/* ── Left column + centre spacer (flex row, vertically centred) ───────── */}
      <div className="relative z-30 flex h-full items-center gap-10 px-14">
        ...
        {/* CENTER — spacer matching photo card footprint */}
        <div className="w-[32%] shrink-0" aria-hidden />

      </div>

      {/* ── Right panel — absolute, anchored at 50 % height ─────────────────── */}
      {/* Starts at the vertical midpoint of the section and flows downward.    */}
      <div
        ref={rightRef}
        className="absolute z-30 flex flex-col gap-5 px-14"
        style={{ top: "40%", right: 0, width: "32%", opacity: 0 }}
      >
        {/* Name */}
        <div
